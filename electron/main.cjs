...
async function capturePrimaryDisplayForOcr() {
  try {
    ...
    if (!source || source.thumbnail.isEmpty()) {
      return { ok: false, message: 'No se pudo capturar la pantalla. Probá con Rocket League en modo ventana sin bordes.', dataUrl: '', width: 0, height: 0, capturedAt: new Date().toISOString() };
    }
    ...
    return { ok: true, message: 'Captura local guardada. Ajustá ROI y confirmá lectura.', dataUrl, width: size.width, height: size.height, capturedAt, filePath };
  } catch (error) {
